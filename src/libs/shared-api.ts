...
const wirteBlob2Clipboard = async(blob: Blob) => await navigator.clipboard.write([new ClipboardItem({[blob.type]: blob})]);

const copyImg = async(target: HTMLDivElement, isMobile?: boolean) => {
	const img = base642blob(await genImg(target));
	if(isMobile) openDialog('检测到你的设备为移动端, 可能不支持将图片复制到剪贴板.\n尽管如此, 你是否仍要复制图片到剪贴板?', {
		confirmCallback: async() => {
			await wirteBlob2Clipboard(img)
				  .then(() => openDialog('图片已复制到剪贴板!'))
				  .catch((e: Error) => openDialog(`复制图片到剪贴板失败: ${e.message}`));
		}
	});
	else await wirteBlob2Clipboard(img)
			   .then(() => openDialog('图片已复制到剪贴板!'))
			   .catch((e: Error) => openDialog(`复制图片到剪贴板失败: ${e.message}`));
}

export {file2base64, base642blob, changeImgs, genImg, copyImg, saveImg, openDialog, wirteBlob2Clipboard};
